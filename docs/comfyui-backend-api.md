...
- **上游把 prompt 弄丢了怎么办**：真实 ComfyUI 中途重启 / OOM 崩溃时，已提交但未执行的 prompt 会消失，`/history/{prompt_id}` 永远查不到。此时若只靠「直到查到历史为止」的轮询，单飞槽位会被永久占用，后续所有任务（含 TTS）全部排队饿死——表现为「一个任务跑了几千秒」。中转站应额外核对真实 `/queue`（这是**中转站内部**调用，插件本身不依赖 `/queue`）：过了宽限期后，若该 `prompt_id` 既不在 `/queue` 也不在 `/history`，即可判定丢失、标记任务失败并释放槽位；同时让 `/history/{prompt_id}` 返回明确的失败条目，插件就能立刻结束轮询而不是等到自己的超时。
